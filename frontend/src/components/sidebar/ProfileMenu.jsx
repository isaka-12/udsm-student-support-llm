...
export default function ProfileMenu({ collapsed, onLogout }) {
  ...
  const MENU_ITEMS = [
    ...
    { Icon: Settings,   label: 'Preferences', action: () => setOpen(false) },
  ];
  ...
  return (
    <div ref={ref} className="relative border-t border-gray-200 dark:border-zinc-800">
      {open && (
        <div className={cn(
          ...
          <div className="px-3 py-2.5 border-b border-gray-100 dark:border-zinc-700/50 mb-1">
            <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">{MOCK_USER.name}</p>
      ...
      {collapsed ? (
        <button onClick={() => setOpen(v => !v)} title={MOCK_USER.name}
        ...
        <button onClick={() => setOpen(v => !v)}
          ...
          <div className="flex-1 min-w-0 text-left">
            <p className="text-sm font-medium text-zinc-800 dark:text-zinc-100 truncate">{MOCK_USER.name}</p>
            <p className="text-xs text-zinc-400 dark:text-zinc-600 truncate">{MOCK_USER.email}</p>
          </div>
